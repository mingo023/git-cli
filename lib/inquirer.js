...
const showPrompt = () => {
	inquirer.prompt(questions).then(answers => {
		...
		let baseScript = `git add ${files.join(
			' ',
		)} && git commit -m '${commitMessage}' && git push --set-upstream origin $(git rev-parse --abbrev-ref HEAD)`;

		const scripts = [baseScript];

		if (isCheckout) {
			scripts.unshift(`git checkout -b ${type}/${branchName}`);
		}
		if (prMessage) {
			scripts.push(`hub pull-request -m '${prMessage}' -o`);
		}
		spinner.start();
		exec(scripts.join(' && '), (err, stdout, stderr) => {
			spinner.stop();
			if (err) {
				console.log(err);
			}
